...
getEvidence(chest_ev, short=FALSE)
@ 
...
chest_cpt <- lapply(flist, function(f){
    cpt(f, levels=yn)}
    )
bn <- compileCPT(chest_cpt) |> grain()

lst <- list(asia=c(1, 99),
            tub=c(5, 95, 1, 99),
            smoke=c(5, 5),
            lung=c(1, 9, 1, 99),            
            bronc=c(6, 4, 3, 7),
...
bn2 <- updateCPT(bn, list(x1=x1_upd))
querygrain(bn2) 
@ 
...
which (internally) corresponds to
<<>>=
c(extractPOT(dat, uG, smooth=e)) |> c()
@ %def
