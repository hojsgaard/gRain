...
```{r chest-grass, fig.height=3, echo=F, fig.cap="Wet graph example; taken from Wikipedia."}
plot(grass_bn)
```
...
specifying the model in \grbn{}.  @lau/spieg:88 motivate the
...
-  A typical use of \bn\ involves setting evidence and then
...
-  Nodes on which evidence is given are not reported unless \code{exclude=FALSE}
...
## Conditioning on evidence with zero probability
{#sec:zero-probabilities}
...
## Hard  and virtual (likelihood) evidence
{#sec:hard-virt-likel}
{#sec:hard-soft}
...
### Extending networks to include other types of variables {#sec:ixture}
...
## Brute force computations and why they fail
{#sec:brute-force-comp}
